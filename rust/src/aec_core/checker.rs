...
mod tests {
    ...
    #[test]
    fn test_csv_quote_value_with_comma() {
        assert_eq!(csv_quote("Smith, John"), "\"Smith, John\"");
    }

    #[test]
    fn test_csv_quote_value_with_double_quote() {
        // Inner " becomes ""
        assert_eq!(csv_quote("say \"hello\""), "\"say \"\"hello\"\"\"");
    }

    #[test]
    fn test_csv_quote_value_with_newline() {
        assert_eq!(csv_quote("line1\nline2"), "\"line1\nline2\"");
    }

    #[test]
    fn test_csv_quote_value_with_spaces_no_quoting() {
    ...
    fn test_checker_args_debug() {
        ...
        // Verify Debug is implemented (no panic)
        let debug_str = format!("{:?}", args);
        assert!(debug_str.contains("input.csv"));
    ...
    fn test_checker_args_clone() {
        let args = CheckerArgs {
            ...
            headless: false,
            dry_run: true,
            nationbuilder_base: "https://example.com".to_string(),
        };
        let cloned = args.clone();
        assert_eq!(cloned.infile, "in.csv");
        assert_eq!(cloned.skip, 5);
        assert_eq!(cloned.threads, 3);
