...
mod tests {
    ...
    // ── convert_state: all full state names ───────────────────────────────────

    #[test]
    fn test_convert_state_all_states() {
        assert_eq!(convert_state("Australian Capital Territory"), "ACT");
        assert_eq!(convert_state("New South Wales"), "NSW");
        assert_eq!(convert_state("Northern Territory"), "NT");
        assert_eq!(convert_state("Queensland"), "QLD");
        assert_eq!(convert_state("South Australia"), "SA");
        assert_eq!(convert_state("Tasmania"), "TAS");
        assert_eq!(convert_state("Victoria"), "VIC");
        assert_eq!(convert_state("Western Australia"), "WA");
    }

    #[test]
    fn test_convert_state_all_abbreviations_already_short() {
        for abbr in &["ACT", "NSW", "NT", "QLD", "SA", "TAS", "VIC", "WA"] {
            assert_eq!(convert_state(abbr), *abbr, "Abbreviation {} should be unchanged", abbr);
        }
    }

    #[test]
    fn test_convert_state_case_insensitive_full_name() {
        assert_eq!(convert_state("victoria"), "VIC");
        assert_eq!(convert_state("VICTORIA"), "VIC");
        assert_eq!(convert_state("Queensland"), "QLD");
    }
    ...
    #[test]
    fn test_convert_state_case_insensitive_abbreviation() {
        assert_eq!(convert_state("vic"), "VIC");
        assert_eq!(convert_state("Nsw"), "NSW");
        assert_eq!(convert_state("qld"), "QLD");
    }

    #[test]
    fn test_convert_state_trims_whitespace() {
        assert_eq!(convert_state("  VIC  "), "VIC");
        assert_eq!(convert_state("  Victoria  "), "VIC");
    }

    #[test]
    fn test_convert_state_unknown_returns_uppercase_input() {
        // Unknown state should come back uppercase
        let result = convert_state("Unknown State");
        assert_eq!(result, "UNKNOWN STATE");
    }

    #[test]
    fn test_convert_state_empty_string() {
        let result = convert_state("");
        assert_eq!(result, "");
    }

    // ── convert_address: state normalization ──────────────────────────────────

    #[test]
    fn test_convert_address_normalizes_state() {
        let (state, _) = convert_address("Victoria", "123 Main Street");
        assert_eq!(state, "VIC");
    }

    #[test]
    fn test_convert_address_street_types() {
        // "Street" → "ST"
        let (_, addr) = convert_address("NSW", "100 King Street");
        assert!(addr.ends_with("ST") || addr.contains("King"), "addr={}", addr);

        // "Avenue" → "AVE"
        let (_, addr) = convert_address("VIC", "10 Collins Avenue");
        assert!(addr.ends_with("AVE") || addr.contains("Collins"), "addr={}", addr);

        // "Drive" → "DR"
        let (_, addr) = convert_address("QLD", "5 Palm Drive");
        assert!(addr.ends_with("DR") || addr.contains("Palm"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_gpo_box_preserved() {
        let (_, addr) = convert_address("VIC", "GPO Box 456");
        assert_eq!(addr, "GPO Box 456");
    }

    #[test]
    fn test_convert_address_locked_bag_preserved() {
        let (_, addr) = convert_address("NSW", "Locked Bag 9000");
        assert_eq!(addr, "Locked Bag 9000");
    }

    #[test]
    fn test_convert_address_po_box_case_insensitive() {
        let (_, addr) = convert_address("VIC", "po box 10");
        assert_eq!(addr, "po box 10");
    }

    #[test]
    fn test_convert_address_circuit_abbreviation() {
        // "Circuit" → "CRCT"
        let (_, addr) = convert_address("ACT", "12 Haydon Circuit");
        assert!(addr.ends_with("CRCT") || addr.contains("Haydon"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_already_abbreviated_street_type() {
        // If street type is already abbreviated (e.g., "ST"), it should be preserved
        let (_, addr) = convert_address("NSW", "100 King ST");
        assert!(addr.contains("ST"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_returns_correct_tuple() {
        let (state, addr) = convert_address("Queensland", "45 Beach Road");
        assert_eq!(state, "QLD");
        assert!(!addr.is_empty());
    }

    #[test]
    fn test_convert_address_court_abbreviation() {
        let (_, addr) = convert_address("VIC", "3 Rose Court");
        assert!(addr.ends_with("CT") || addr.contains("Rose"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_crescent_abbreviation() {
        let (_, addr) = convert_address("NSW", "7 Sunset Crescent");
        assert!(addr.ends_with("CRES") || addr.contains("Sunset"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_place_abbreviation() {
        let (_, addr) = convert_address("VIC", "2 Market Place");
        assert!(addr.ends_with("PL") || addr.contains("Market"), "addr={}", addr);
    }

    #[test]
    fn test_convert_address_close_abbreviation() {
        let (_, addr) = convert_address("WA", "8 River Close");
        assert!(addr.ends_with("CL") || addr.contains("River"), "addr={}", addr);
    }

    // ── Regression: street name with number prefix ────────────────────────────

    #[test]
    fn test_convert_address_with_leading_house_number() {
        // The address should still produce a valid normalized output
        let (state, addr) = convert_address("VIC", "42 Smith Street");
        assert_eq!(state, "VIC");
        assert!(!addr.is_empty());
    }
}
