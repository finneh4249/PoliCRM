...
import { ArrowLeft, Map as MapIcon, BarChart3, TrendingUp, Users, CheckCircle2, Target } from 'lucide-react';
...
export function WarRoom() {
    ...
    const [geoData, setGeoData] = useState<GeographicData>({ by_state: {}, by_division: {} });
    ...
    const style = (feature: any) => {
        const name = feature.properties.electorateName || feature.properties.elect_div || feature.properties.Name || feature.properties.NAME;
    ...
    const verificationRate = totalMembers > 0 ? ((totalVerified / totalMembers) * 100).toFixed(1) : '0.0';

    // Format growth data for chart
    const growthChartData = Object.entries(growthData).map(([month, count]) => ({
        month,
        members: count
                    ...
                    {activeView === 'map' && (
                        <div className="flex items-center gap-2 bg-slate-800 p-1 rounded-lg">
                            <button
                                ...
                                className={`px-3 py-1 rounded-md text-sm transition-colors ${viewMode === 'verified' ? 'bg-green-600 text-white' : 'text-slate-400 hover:text-white'}`}
                            >
                                Verified
                            </button>
                            <button
                                onClick={() => setViewMode('projected')}
                                className={`px-3 py-1 rounded-md text-sm transition-colors ${viewMode === 'projected' ? 'bg-yellow-600 text-white' : 'text-slate-400 hover:text-white'}`}
                            >
                                Projected
                            </button>
                            <button
                                onClick={() => setViewMode('combined')}
                                className={`px-3 py-1 rounded-md text-sm transition-colors ${viewMode === 'combined' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                            >
                                Combined
                            </button>
                        </div>
                ...
                {loading ? (
                    ...
                    <div className="h-full relative">
                        ...
                        <div className="absolute top-4 right-4 z-[1000] bg-slate-800/95 backdrop-blur-sm p-4 rounded-lg border border-slate-700 shadow-xl">
                            <div className="text-sm font-semibold mb-3 text-slate-300">Legend</div>
                            <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                    <span className="w-4 h-4 rounded-full bg-[#006d2c]"></span>
                                    <span className="text-xs text-slate-400">Verified (High)</span>
                                </div>
                                <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-slate-400">Projected (High)</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className="w-4 h-4 rounded-full bg-[#FD8D3C]"></span>
                                    <span className="text-xs text-slate-400">Projected (Med)</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="h-full overflow-y-auto p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                ...
                                <div className="text-3xl font-bold text-white">{totalMembers.toLocaleString()}</div>
                            </div>
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                ...
                                <div className="text-3xl font-bold text-yellow-400">{totalProjected.toLocaleString()}</div>
                            </div>
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="text-3xl font-bold text-blue-400">{verificationRate}%</div>
                            </div>
                        </div>

                        {/* Growth Chart */}
                        <div className="bg-slate-800 rounded-lg p-6 border border-slate-700 mb-6">
                            <h2 className="text-xl font-semibold mb-4 text-slate-200">Member Growth</h2>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={growthChartData}>
                                    ...
                                    />
                                    <Legend />
                                    <Line type="monotone" dataKey="members" stroke="#6366f1" strokeWidth={2} dot={{ fill: '#6366f1' }} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* State Distribution */}
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                <h2 className="text-xl font-semibold mb-4 text-slate-200">Distribution by State</h2>
                                <ResponsiveContainer width="100%" height={300}>
                                    <BarChart data={stateChartData}>
                                        <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                                        <XAxis dataKey="state" stroke="#94a3b8" style={{ fontSize: '12px' }} />
                                        <YAxis stroke="#94a3b8" style={{ fontSize: '12px' }} />
                                        <RechartsTooltip
                                            contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', borderRadius: '6px' }}
                                            labelStyle={{ color: '#f1f5f9' }}
                            ...
                            {/* Top Electorates */}
                            <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
                                <h2 className="text-xl font-semibold mb-4 text-slate-200">Top 10 Electorates</h2>
                                <div className="space-y-3">
                                    {topElectorates.map(([name, count], index) => (
                                        <div key={name} className="flex items-center justify-between">
                                            ...
                                            <div className="text-green-400 font-semibold">{count}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
